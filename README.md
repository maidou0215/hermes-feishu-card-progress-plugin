...
安装后：
- API 请求增加 `card_msg_content_type=raw_card_content` 参数，获取卡片原始 JSON
- 解析 `json_card` 包裹的 schema 1.0/2.0 卡片结构，提取实际文本内容
- Reply chain 不截断引用内容（需要上游补丁），与 cc-connect 行为一致
...
插件需要对 Hermes 核心文件打 **2 处** 补丁（均在 `gateway/run.py`）：
1. 让 card 模式跳过 reasoning 拼接
2. Reply chain 不截断引用内容（与 cc-connect 行为对齐）
...
Hermes 更新（`git pull`）后，只需检查 2 处补丁是否被覆盖：

```bash
cd ~/.hermes/hermes-agent
git pull origin main
grep -n 'FEISHU_PROGRESS_STYLE' gateway/run.py  # 补丁 1：应有结果
