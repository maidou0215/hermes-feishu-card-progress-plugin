...
启用 `streaming.enabled: true` 后，模型回复以 `post` 格式逐步更新，进度卡片（`interactive` 格式）并行工作：

- 处理期间 `_build_outbound_payload` 跳过 interactive 转换，避免飞书 PATCH 的 msg_type 变更限制
- 流式文本和进度卡片互不干扰，各自独立更新
- 处理完成后恢复 interactive 卡片格式（非流式场景）
...
### 3. Thinking/Reasoning 实时显示

模型的推理过程实时显示在进度卡片中：

- 灰色 notation 文本 + 💭 前缀，与工具条目视觉区分
- 保留所有 reasoning 条目（受 10 条上限截断，与 cc-connect 行为对齐）
...
在 profile 的 `config.yaml` 中启用流式，让模型回复逐步更新而非等待完成后一次性发送：
...
| FeishuAdapter | `edit_message()` | 拦截进度更新 → PATCH 卡片；非进度文本绕过 interactive 转换 |
| FeishuAdapter | `_build_outbound_payload` | Markdown 响应使用 Schema 2.0 卡片格式；处理期间跳过（流式用 post 格式） |
...
LLM response
  ...
  │
  ├─ stream delta ──→ StreamConsumer
  │                    ├─→ adapter.send() ──→ _patched_build_outbound_payload
  │                    │                     └─→ post 格式（处理期间跳过 interactive）
  │                    └─→ adapter.edit_message() ──→ _patched_edit_message
  │                                                      └─→ post 格式更新
  │
...
| 流式文本预览 | `streamPreview` 有节流 | `post` 格式逐步更新（无节流） |
